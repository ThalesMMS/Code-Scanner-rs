...
IGNORE_CONTENT_FILES = {"Pipfile.lock", ".DS_Store", "listfiles.py", "estrutura_pastas.txt", ".gitignore"}

# Extensions whose CONTENT will be ignored
IGNORE_CONTENT_EXTENSIONS = {".svg", ".png", ".jpg", ".jpeg", ".gif", ".webp", ".ico", ".woff", ".woff2", ".ttf", ".otf", ".pyc", ".lock"}

# Directories to ignore at root level
IGNORE_DIRS_ROOT = {".git", ".vscode", "venv", ".venv", "env", "dist", "build", "input", "output"}

# Directories to ignore at ANY level
IGNORE_DIRS_ANYWHERE = {"__pycache__", "node_modules", ".pytest_cache", ".mypy_cache", ".tox", "htmlcov", "coverage"}
...
PROFILE = SummaryProfile(
    target_subdirs={DEFAULT_TARGET_SUBDIR},
    ...
    ignore_any_dirs=IGNORE_DIRS_ANYWHERE,
    ignore_any_files=IGNORE_FILES_ANYWHERE,
    max_file_size=MAX_FILE_SIZE_BYTES,
    treat_filename_as_extension=True,
)
...
def process_project(project_dir: Path, output_filename: Path, target_subdirs: Set[str]) -> bool:
    ...
    print(f"  Target subdirectory for deep analysis: {', '.join(sorted(target_subdirs))}")
    print(f"  Extensions/Files with content included: {', '.join(sorted(CODE_EXTENSIONS))}")
    ...
    print(f"  Directories ignored anywhere: {', '.join(sorted(IGNORE_DIRS_ANYWHERE))}")
    print(f"  Files ignored anywhere: {', '.join(sorted(IGNORE_FILES_ANYWHERE))}")
    ...
    try:
        ...
        with open(output_filename, "w", encoding="utf-8") as outfile:
            ...
            if IGNORE_CONTENT_FILES:
                ignored_info.append(f"files like {', '.join(sorted(IGNORE_CONTENT_FILES))}")
            if IGNORE_CONTENT_EXTENSIONS:
                ignored_info.append(f"extensions {', '.join(sorted(IGNORE_CONTENT_EXTENSIONS))}")
            if ignored_info:
...
def main() -> int:
    ...
    target_subdir_value = os.environ.get("TARGET_SUBDIR", DEFAULT_TARGET_SUBDIR)
    target_subdirs = {target_subdir_value}
    ...
    print(f"Target subdirectory: {', '.join(sorted(target_subdirs))}")
    print("=" * 60)
    print()
    ...
    for project_path in sorted(projects):
        output_filename = output_dir / f"{project_path.name}_django_summary.txt"
