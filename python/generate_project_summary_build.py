...
INCLUDE_ROOT_FILES_BY_NAME = {"eslint.config.js", "vite.config.js", "index.html", "package.json", "README.md"}
...
IGNORE_DIRS_ROOT = {"node_modules", "dist", "build", ".git", ".vscode", "__pycache__", "input", "output"}
...
PROFILE = SummaryProfile(
    ...
    allow_bare_without_extension=True,
)
...
def process_project(project_dir: Path, output_filename: Path, target_subdirs: Set[str]) -> bool:
    ...
    print("  Files without extension will be checked for binary nature")
    ...
    try:
        ...
        with open(output_filename, "w", encoding="utf-8") as outfile:
            ...
            outfile.write("(Binary files are ignored)\n\n")
...
def main() -> int:
    ...
    for project_path in sorted(projects):
        output_filename = output_dir / f"{project_path.name}_build_summary.txt"
